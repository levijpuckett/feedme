...
#        distance = max_dist
#        for food in foods:
#            cur_dist = self._calc_distance(self.headbody.center, food.center)
#            if cur_dist < distance:
#                distance = cur_dist
#        self.nom += 1 - (distance / max_dist)
...
#        danger_dist, danger_coords = self.find_nearest_item(dangers)
        # Find the nearest neighbour.
#        if neighbours:
#            neighbour_dist, neighbour_coords = self.find_nearest_item(neighbours)
...
#        danger_coords = (danger_coords[0] - self.headbody.centerx,
#                         danger_coords[1] - self.headbody.centery)
        food_coords = ( food_coords[0]/(1+abs(food_coords[0])),
                       food_coords[1]/(1+abs(food_coords[1])))

#        danger_coords = ( math.sin(10*danger_coords[0])/(-danger_coords[0])+10,
#                       math.sin(10*danger_coords[0])/(-danger_coords[0])+10)
#        danger_dist = self._danger_mod(danger_dist)
#        brain_input = np.array(food_coords + tuple([danger_dist]))
...
#        if self.headbody.collidelistall(dangers):
#            self.num_foods_eaten = 0
#            self.dead = True
...
#        print (brain_input, speed, self.max_speed)
        return foods_eaten
